...
void TSWidget::initializeGL()
{
	...
	//std::vector<QString> pathims;
	//for(int i=0;i<3;i++)
	//	pathims.push_back( QFileDialog::getOpenFileName(this,"Open Image", "/Users/paulinus/pro/mv/data", "Image Files (*.png)") );
	//loadImageFile(pathims);	
	int inum = pathImage.size();
	
	frames.resize(inum);
	for(int i=0;i<inum;i++)
	{
		frames[i].set_filenames( std::string(pathImage[i].toAscii().data()) );
...
void TSWidget::paintGL()
{
	...
	for( int i=0; i<inum; i++ ) {
		...
		glPointSize(3);
		for(unsigned int j=0; j<frames[i].features.size(); j++)
		{
			SiftFeature &sf = frames[i].features[j];
