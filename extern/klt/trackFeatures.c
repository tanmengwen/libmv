...
static int _trackFeature(
  ...
  float *y2,
  float *absdiff,
  float *gxx_,  /* The user is likely interested in the curvature */
  float *gxy_,  /* of the cost function. Since we compute it any, */
  float *gyy_,  /* pass it back to our caller. */
  ...
  int lighting_insensitive)  /* whether to normalize for gain and bias */
{
  ...
  do  {
    ...
    *y2 += dy;
	...
	/* Save the computed curvature for the future. */
	*gxx_ = gxx/(width*width);
	*gxy_ = gxy/(width*height);
	*gyy_ = gyy/height/height;
  ...
  if (status == KLT_TRACKED)  {
    if (lighting_insensitive)
      ...
      _computeIntensityDifference(img1, img2, x1, y1, *x2, *y2, 
                                  width, height, imgdiff);
    *absdiff = _sumAbsFloatWindow(imgdiff, width, height)/(width*height);
    if (*absdiff > max_residue) 
...
static int _am_trackFeatureAffine(
		...
		float *x2, /* starting location of search in second image */
		float *y2,
		float *absdiff,  /* absdiff of image patches after tracking */
		float *gxx_,  /* For non-affine check; returns shape of cost function */
		float *gxy_,  /* these are ignored if affine_map != 0 */
		float *gyy_,  
		_KLT_FloatImage img1, 
		_KLT_FloatImage gradx1,
		_KLT_FloatImage grady1,
 ...
 * KLTClearFeature
 *
 * Clears a feature and frees associated affine memory, but does
 * not free the feature itself. 
 */

void KLTClearFeature(KLT_Feature feature) {
	feature->x = -1.0;
	feature->y = -1.0;
	feature->aff_x = -1.0;
	feature->aff_y = -1.0;
	feature->val = -1;
	if( feature->aff_img ) {
		_KLTFreeFloatImage(feature->aff_img);
		feature->aff_img = NULL;
	}
	if( feature->aff_img_gradx ) {
		_KLTFreeFloatImage(feature->aff_img_gradx);
		feature->aff_img_gradx = NULL;
	}
	if( feature->aff_img_grady ) {
		_KLTFreeFloatImage(feature->aff_img_grady);
 ...
 * KLTTrackOneFeature
 *
 * Tracks one feature point given the appropriate image pyramids.
 */

void KLTTrackOneFeature(KLT_TrackingContext tc, KLT_Feature feature)
{
	float xloc = feature->x;
	float yloc = feature->y;
	float xlocout, ylocout, absdiff=0;
	float subsampling = tc->subsampling;
	int val=-1;
	int r;
	_KLT_Pyramid pyramid1       = tc->pyramid_last;
	_KLT_Pyramid pyramid1_gradx = tc->pyramid_last_gradx;
	_KLT_Pyramid pyramid1_grady = tc->pyramid_last_grady;
	_KLT_Pyramid pyramid2       = tc->pyramid_current;
	_KLT_Pyramid pyramid2_gradx = tc->pyramid_current_gradx;
	_KLT_Pyramid pyramid2_grady = tc->pyramid_current_grady;
	int nrows = pyramid1->img[0]->nrows;
	int ncols = pyramid1->img[0]->ncols;
	...
	if (val == KLT_OOB
			|| val == KLT_SMALL_DET
			|| val == KLT_LARGE_RESIDUE
			|| val == KLT_MAX_ITERATIONS) {
		feature->val = val;
		KLTClearFeature(feature);
		...
		feature->x = xlocout;
		feature->y = ylocout;
		feature->absdiff = absdiff;
		feature->val = KLT_TRACKED;
		if (tc->affineConsistencyCheck >= 0 && val == KLT_TRACKED)  { /*for affine mapping*/
			...
			if(!feature->aff_img) {
				...
				feature->aff_img = _KLTCreateFloatImage((tc->affine_window_width +border),
				                                        (tc->affine_window_height+border));
				feature->aff_img_gradx = _KLTCreateFloatImage((tc->affine_window_width+border),
				                                              (tc->affine_window_height+border));
				feature->aff_img_grady = _KLTCreateFloatImage((tc->affine_window_width+border),
				                                              (tc->affine_window_height+border));
				_am_getSubFloatImage(pyramid1->img[0], xloc,yloc,feature->aff_img);
				_am_getSubFloatImage(pyramid1_gradx->img[0], xloc,yloc,feature->aff_img_gradx);
				_am_getSubFloatImage(pyramid1_grady->img[0], xloc,yloc,feature->aff_img_grady);
				feature->aff_x = xloc - (int) xloc + (tc->affine_window_width+border)/2;
				feature->aff_y = yloc - (int) yloc + (tc->affine_window_height+border)/2;;
			} else {
				/* affine tracking */
				val = _am_trackFeatureAffine(feature->aff_x, feature->aff_y,
					&xlocout, &ylocout, &absdiff,
					&feature->aff_grad_xx,
					&feature->aff_grad_xy,
					&feature->aff_grad_yy,
					feature->aff_img, 
					feature->aff_img_gradx, 
					feature->aff_img_grady,
					pyramid2->img[0], 
...
static inline void _swapPtrs(void **a, void **b) {
	void *tmp = *a;
	*a = *b;
	*b = tmp;
}

static void _allocateImagePyramidsAndGradients(
...
{
	*pyr = _KLTCreatePyramid(ncols, nrows, (int) tc->subsampling, tc->nPyramidLevels);
	*pyr_gradx = _KLTCreatePyramid(ncols, nrows, (int) tc->subsampling, tc->nPyramidLevels);
	*pyr_grady = _KLTCreatePyramid(ncols, nrows, (int) tc->subsampling, tc->nPyramidLevels);
}

/*********************************************************************
 * Computes relevant image pyramids and gradients.
 *
 * The resulting size is taken from tmpimg, which should match the
 * size (and contents) of img.
 */
static void _initImagePyramidsAndGradients(KLT_TrackingContext tc,
		KLT_PixelType *img,
		...
		_KLT_Pyramid pyr_grady)
{
	...
	_KLTComputeSmoothedImage(tmpimg, _KLTComputeSmoothSigma(tc), pyr->img[0]);
	_KLTComputePyramid(NULL, pyr, tc->pyramid_sigma_fact);
	for (i = 0 ; i < tc->nPyramidLevels ; i++) {
		_KLTComputeGradients(pyr->img[i], tc->grad_sigma, 
		pyr_gradx->img[i],
		pyr_grady->img[i]);
	}
					  ...
					  KLT_FeatureList featurelist)
{
	...
	if (!tc->tmpimage) {
		tc->tmpimage = tmpimg = _KLTCreateFloatImage(ncols, nrows);
	}
	tmpimg = tc->tmpimage;

	/* Re-use pyramids and memory across calls */
	if (tc->pyramid_current != NULL && tc->pyramid_last != NULL)  {

		assert(tc->pyramid_current_gradx != NULL);
		assert(tc->pyramid_current_grady != NULL);
		assert(tc->pyramid_last_gradx != NULL);
		assert(tc->pyramid_last_grady != NULL);

		_swapPtrs(&tc->pyramid_last, &tc->pyramid_current);
		_swapPtrs(&tc->pyramid_last_gradx, &tc->pyramid_current_gradx);
		_swapPtrs(&tc->pyramid_last_grady, &tc->pyramid_current_grady);

		pyramid1 = tc->pyramid_last;
		pyramid1_gradx = tc->pyramid_last_gradx;
		pyramid1_grady = tc->pyramid_last_grady;

		pyramid2 = tc->pyramid_current;
		pyramid2_gradx = tc->pyramid_current_gradx;
		pyramid2_grady = tc->pyramid_current_grady;

		assert(pyramid1->ncols[0] == ncols);
		assert(pyramid1->nrows[0] == nrows);
		assert(pyramid2->ncols[0] == ncols);
		assert(pyramid2->nrows[0] == nrows);
	} else if (tc->pyramid_last == NULL
			&& tc->pyramid_current == NULL) {

		_allocateImagePyramidsAndGradients(tc, ncols, nrows, &pyramid1, &pyramid1_gradx, &pyramid1_grady);
		tc->pyramid_last = pyramid1;
		tc->pyramid_last_gradx = pyramid1_gradx;
		tc->pyramid_last_grady = pyramid1_grady;
		_initImagePyramidsAndGradients(tc, img1, tmpimg, pyramid1, pyramid1_gradx, pyramid1_grady);

		_allocateImagePyramidsAndGradients(tc, ncols, nrows, &pyramid2, &pyramid2_gradx, &pyramid2_grady);
		tc->pyramid_current = pyramid2;
		tc->pyramid_current_gradx = pyramid2_gradx;
		tc->pyramid_current_grady = pyramid2_grady;
	}

	_initImagePyramidsAndGradients(tc, img2, tmpimg, pyramid2, pyramid2_gradx, pyramid2_grady);
	...
	/* Track each feature across frames, ignoring lost features */
	for (indx = 0 ; indx < featurelist->nFeatures ; indx++)  {
		if (featurelist->feature[indx]->val >= 0)  {
			KLTTrackOneFeature(tc, featurelist->feature[indx]);
		}
	}
