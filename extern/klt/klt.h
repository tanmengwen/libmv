...
typedef struct  {
  ...
  */
  ...
  void *pyramid_current;
  void *pyramid_current_gradx;
  void *pyramid_current_grady;
  void *tmpimage;
...
typedef struct  {
	...
	KLT_locType y;
	float absdiff;
	/* gradient sums; roughly represents the steepness of the error surface at the
	 * final KLT iteration. */
	float grad_xx;
	float grad_xy;
	float grad_yy;
	...
	float aff_grad_xx; /* this is ONLY for affine consistency check with affine */
	float aff_grad_xy; /* turned off. This is allows to estimate uncertainty */
	float aff_grad_yy; /* across frames. */
	KLT_locType aff_x;
	KLT_locType aff_y;
	KLT_locType aff_Axx;
	KLT_locType aff_Ayx;
	KLT_locType aff_Axy;
	KLT_locType aff_Ayy;
	float aff_absdiff; /* abs diff for affine */
