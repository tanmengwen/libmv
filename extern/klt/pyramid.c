...
/*********************************************************************
 * Computes the image pyramid. If img is null, assumes the base level is
 * already the image to be built into the pyramid.
 *
 */
  ...
  float sigma_fact)
{
  ...
  if (img) {
	  ...
	  ncols = pyramid->ncols[0];
	  nrows = pyramid->nrows[0];
  }
  ...
  for (i = 1 ; i < pyramid->nLevels ; i++)  {
	/* Hack to avoid reallocating big images */
	tmpimg->nrows = nrows;
	tmpimg->ncols = ncols;
